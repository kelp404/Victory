...
(function() {
    ...
    */
    ...
    ViewEventsApplication.prototype.show_modal_for_add_application = function() {
      ...
      */
      ...
      return $(document).on('click', 'a[href="#form_add_application"]', function() {
        $('#name').val($('.appended_input_application').val());
        setTimeout("$('#name').focus()", 500);
      });
    };

    ViewEventsApplication.prototype.add_application = function() {
      /*
      add an application.
      :param url: $(@).attr('action')
      :param data: $(@).serialize()
      */
      return $(document).on('submit', 'form#form_add_application', function() {
  ...
  ViewEventsUser = (function() {
    ...
    */
    ...
    ViewEventsUser.prototype.add_user = function() {
      /*
      add a new user to Takanashi.
      :param url: $(this).attr('action')
      :param data: $(this).serialize()
      */
      return $(document).on('submit', 'form#form_add_user', function() {
        if (!core.validation($(this))) {
          return false;
        }
        $.ajax({
        ...
        $.ajax({
          ...
          success: function(r) {
            if (r.success) {
              ...
              return $($('.profile p')[0]).text(result.name);
            } else {
              return KNotification.pop({
                title: 'Failed!',
                message: 'Please check again.'
